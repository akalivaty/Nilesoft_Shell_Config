...
item
(
	type='dir|drive|desktop|back.directory|back.drive'
	where=package.exists("WindowsTerminal") && (sel.count or wnd.is_taskbar or wnd.is_edit) 
	title=title.Windows_Terminal 
	tip=tip_run_admin 
	admin=has_admin 
	image='@package.path("WindowsTerminal")\WindowsTerminal.exe' cmd="wt.exe" arg=`-d "@sel.path\."`
)